...
nn_codellama_rmsnorm <- nn_module(
...
codellama_rotate_half <- function(x) {
  c(x1, x2) %<-% torch_split(x, x$size(-1) / 2, -1)
  torch_cat(list(-x2, x1), dim = -1)
}

nn_codellama_rotary_embedding <- nn_module(
  ...
  forward = function(x) {
    ...
    x * cos + codellama_rotate_half(x) * sin
  }
)

nn_codellama_attention <- nn_module(
  initialize = function(n_head, n_embd, max_pos, rope_base) {
    self$n_head <- n_head
    self$n_embd <- n_embd

    self$max_pos <- max_pos
    self$rotary <- nn_codellama_rotary_embedding(n_embd/n_head, max_pos, rope_base)
...
nn_codellama_layer <- nn_module(
  initialize = function(n_embd, n_inter, n_head, max_pos, rmsnorm_eps, rope_base) {
    self$ln_1 <- nn_codellama_rmsnorm(n_embd, rmsnorm_eps)
    self$ln_2 <- nn_codellama_rmsnorm(n_embd, rmsnorm_eps)
    self$attn <- nn_codellama_attention(n_head, n_embd, max_pos, rope_base)
    self$mlp <- nn_codellama_mlp(n_embd, n_inter)
  },
  forward = function(x) {
...
nn_codellama_model <- nn_module(
  initialize = function(vocab_size, n_embd, n_inter, n_head, n_layer, max_pos,
    ...
    self$transformer <- nn_module_dict(list(
      ...
      h = nn_sequential(!!!map(
        1:n_layer,
        \(x) nn_codellama_layer(n_embd, n_inter, n_head, max_pos, rmsnorm_eps, rope_base)
      )),
      ln_f = nn_codellama_rmsnorm(n_embd, rmsnorm_eps)
    ))
...
#' codellama
#'
#' Initializes a codellama like model
#'
...
#' @export
codellama <- function(vocab_size=50432, n_embd=6144, n_inter = 11008, n_head=64,
                  n_layer=44, max_pos=2048, rmsnorm_eps = 1e-6, rope_base = 10000) {
  nn_codellama_model(vocab_size, n_embd, n_inter, n_head, n_layer, max_pos, rmsnorm_eps,
                 rope_base)
}

#' @describeIn codellama Initializes a codellama model using a configuration defined in HF Hub
#' @export
codellama_from_config <- function(identifier, revision = "main") {
  path <- hfhub::hub_download(identifier, "config.json", revision = revision)
  config <- jsonlite::fromJSON(path)

  if (config$model_type != "llama")
    cli::cli_abort(c(
      "{.arg config$model_type} must be {.val codellama}, got {.val {config$model_type}}"
    ))
  ...
  rope_base   <- config$rope_theta # unlike LLama, code llama models tune this

  codellama(vocab_size, n_embd, n_inter, n_head, n_layer, max_pos, rmsnorm_eps, rope_base)
}

#' @describeIn codellama Initializes the codellama model and load pre-trained weights from HF hub.
#' @export
codellama_from_pretrained <- function(identifier, revision = "main") {
  with_device(device="meta", {
    model <- codellama_from_config(identifier, revision)
  })
  state_dict <- hf_state_dict(identifier, revision)
  state_dict <- codellama_hf_weights_remap(state_dict)

  model$load_state_dict(state_dict, .refer_to_state_dict = TRUE)
  model
}

codellama_hf_weights_remap <- function(state_dict) {
